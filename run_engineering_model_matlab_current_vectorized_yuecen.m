function [T I0 I0_T I1_T I2_T EV_TOTAL HPHI_TOTAL EH_TOTAL]=run_engineering_model_matlab_current_vectorized_yuecen...
...
H0_vector=zeros(NBE,1);
[CUR_0,DCUR_0]=CURRENT(TE_vectorized,H0_vector, RZ, HR, RR, RG, ALPHA_TOWER, NC_vectorized, VC,C, MODEL);
...
% ===========Z GREATER THAN HR_number 
	    ...
	    if (B1>=0) 
% 	     if (MODEL_number == 5) 
% B1 = B1 + (QUO_vector(ii)-HR_number)/VC_number + (QUO_vector(ii)-HR_number)/C;
%          end 
		 CUR=CUR+IPULSE(B1)*P(QUO_vector(ii))+sum_expo(B21,B22);
	     DCUR=DCUR+DIPULSE(B1)*P(QUO_vector(ii))+dsum_expo(B21,B22);
         if or(not(isreal(CUR)),not(isreal(DCUR)))
            disp('Non-real value on the current routine') 
         end
        end
...
n_max =3;
for n= 1:n_max
    update1 = (-1)^(n+1)*RG^n*IPULSE(k^(n-1)*t1);
    tot1= tot1+update1;
    
   update2 = (-1)^n*RG^n*IPULSE(k^n*t2);
   tot2 = tot2+update2;
...
n_max =3;
for n= 1:n_max
 update1 = (-1)^(n+1)*RG^n*k^(n-1)*DIPULSE(k^(n-1)*t1);
 tot1= tot1+update1;
    
 update2 = (-1)^n*RG^n*k^n*DIPULSE(k^n*t2);
 tot2 = tot2+update2;
...
function inttot1=intsum_expo1(A,B)
global C RG V
k = (C-V)/(C+V);
tot1=0;
...
function inttot2=intsum_expo2(A,B)
global C RG V
k = (C-V)/(C+V);
tot2=0;
...
%===========x GREATER THAN HR
    ...
    B1(B1<=0)=0; %Carlos added for solving one problem
	    if any(B1>=0)
          ...
          B22=TT-RZ/C+x/C;
	      ICII=INTPULSE(A3,B3);
		  ICI=ICI+ICII.*P(x)+intsum_expo1(A3,B21)+intsum_expo2(A3,B22);
        end
%===========2
% 	B2=TT-RZ/C-(x-HR)/C;
%     B2(B2<=0)=0; %Carlos added for solving one problem
...
%         else
    %===========x GREATER THAN HR
            ...
            if any(B1>=0) 
                ICII=IPULSE(B3);
                ICI=ICI+ICII.*P(x)+sum_expo(B21,B22);
            end
%     %===========2
...
%===========x GREATER THAN HR
    ...
    B22(B22<=0)=0;
	    if any(B1>=0)
            ICII=DIPULSE(B3);
            ICI=ICI+ICII.*P(x)+dsum_expo(B21,B22);
            if not(isreal(ICI))
              disp('Non-real value on the ERADP routine')  
            end
		end
