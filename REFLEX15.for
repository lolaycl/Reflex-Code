...
c      L'instruction global permet le regroupement de zones mémoires pouvant
c      être partagées par différentes unités de programme (fonctions, procédures).
c      When a subroutine or function is exited, local variables become undefined.
c      The same thing may happen with the variables stored in named COMMON blocks.
c      Therefore, it is possible to SAVE an entire named COMMON block
c      (but not individual variables in the block) in a procedure with the command
c      SAVE /named common block1/, /named common block2/, ..., /named common blockn/
c      Variables in a blank COMMON block never become undefined since the it
c      is declared in the main program. Similarly,
c      if a named COMMON block is declared in the main program,
c      then it is unecessary to use the SAVE command in other program units.
c      Note that you cannot use the SAVE command on a blank COMMON block.
...
C	Turn-On Term fields
	...
	B=2*ZS/(C*C)-2*HR/(V*V)-2*TT/V
	CC=TT*TT+2*TT*HR/V+HR*HR/(V*V)-R*R/(C*C) 
C el punto H visto desde el punto de observacion
	RADIC=DABS(B*B-4*A*CC)
	Z1max=(-B-DSQRT(RADIC))/(2*A)
	R1=DSQRT(R*R+(ZS-Z1max)**2)
	RZ=R1
...
c the global variables that attain this function should be declared as well here
	IMPLICIT NONE
	...
	COMMON /COM6/ HR,RR,RG,ALPHA_TOWER,NC,MODEL,MPROBLEM
      COMMON /COM2/ V,VC,W,H,ZLAM
	COMMON /COM1/ MU,C,PI,EPS
C     An EXTERNAL statement is used to identify a symbolic name as representing an external procedure or dummy procedure, and to permit such a name to be used as an actual argument
