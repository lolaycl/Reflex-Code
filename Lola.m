function [T I0 I0_T I1_T I2_T EV_TOTAL HPHI_TOTAL EH_TOTAL]=Lola...
...
% 'TCS'    MODEL = 5;
    ...
    NBE2=round((TMAX-TINT)/(DELTATE1)); % ???DELTATE2=DELTATE1*DTS  
...
% TIMEE=TE_vectorized;
% 
% TE1_vectorized_1=DELTATE1:DELTATE1:(NBE1*DELTATE1);
% TE2_vectorized_2=(NBE1+1)*DELTATE1:DELTATE2:((NBE2*DELTATE2+NBE1*DELTATE1)-DELTATE2);
% TE_vectorized_1=[TE1_vectorized_1, TE2_vectorized_2];

N=1:1:NBE;
DELTATE=DELTATE1;
TE_vectorized=0:DELTATE:(NBE*DELTATE-DELTATE);
TIMEE=TE_vectorized;

% TE_vectorized_1=DELTATE:DELTATE:(NBE*DELTATE-2*DELTATE);

% TE_vectorized_2=2:1:(NBE-1);
% TE_vectorized_2=(2*DELTATE):DELTATE:(NBE*DELTATE-DELTATE);
NC_vectorized=(2*C/V).*ones(NBE,1);
% NC_vectorized=3.*ones(NBE,1);
% NC_vectorized=(C/(V))./(TE_vectorized+1).*(2.*TE_vectorized+1);


% NC_vectorized = [0;NC_vectorized_1];

    
%     NC_vectorized_1=zeros(1,20);
%     NC_vectorized_2=2.*ones(1,(NBE-20));
%     NC_vectorized=[NC_vectorized_1, NC_vectorized_2];
...
H_0_vector=zeros(NBE,1);
[CUR_0,DCURT_0]=CURRENT(TE_vectorized,H_0_vector, RZ, HR, RR, RG, ALPHA_TOWER, NC_vectorized, VC,C, MODEL);
...
function [CUR_out,DCUR_out]=CURRENT(TY_vector, QUO_vector, RZ_vector, HR_number, RR_number, RG_number, ALPHA_TOWER_number, NC_vector, VC_number,C_number, MODEL_number)
  ...
  for ii=1:numel(NC_vector) %For each element of NC vector execute
    ...
    CUR=0;
    DCUR=0;    
    k = (C_number-V)/(C_number+V);
    B3=TY_vector(ii)-RZ_vector(ii)/C_number-QUO_vector(ii)/V;
    
        B1=TY_vector(ii)-RZ_vector(ii)/C_number-QUO_vector(ii)/VC_number;       
        if (B3>=0) 
		 CUR=CUR+IPULSE(B1)*P(QUO_vector(ii));
	     DCUR=DCUR+DIPULSE(B1)*P(QUO_vector(ii));
         if or(not(isreal(CUR)),not(isreal(DCUR)))
            disp('Non-real value on the current routine') 
         end
        end

        for J=1:1:NC_vector(ii)
          B21=TY_vector(ii)-RZ_vector(ii)/C_number-QUO_vector(ii)/C_number;
          B22=TY_vector(ii)-RZ_vector(ii)/C_number+QUO_vector(ii)/C_number;
              if (B3>=0)
                  update1 = (-1)^(J+1)*RG_number^(J)*IPULSE(k^(J-1)*B21);
                  update2 = (-1)^(J)*RG_number^(J)*IPULSE(k^(J)*B22);
                 CUR=CUR+(update1+update2).*exp(-(J+1)*HR_number*ALPHA_TOWER_number);
                 DCUR=DCUR+(k^(J+1)*update1+k^(J)*update2).*exp(-(J+1)*HR_number*ALPHA_TOWER_number);
                 if or(not(isreal(CUR)),not(isreal(DCUR)))
                    disp('Non-real value on the current routine') 
                end
              end
...
%	=============================
    ...
    B1(B1<=0)=0; %Carlos added for solving one problem
	   ...
	   for J=1:1:NC
          B21=TT-RZ/C-x/C;
          B22=TT-RZ/C+x/C;
          B21(B21<=0)=0; %Carlos added for solving one problem
          B22(B22<=0)=0;
          if (B21>=0)
	         A2=0;
             B21=k^(J-1)*B21;
             B22=k^(J)*B22;
             ICII_1=INTPULSE(A2,B21);
             ICII_2=INTPULSE(A2,B22);
		     ICI=ICI+(ICII_1.*(-1)^(J+1)*RG^(J)*k^(1-J)+ICII_2.*(-1)^(J)*RG^(J)*k^(-J))*exp(-(J+1)*HR.*ALPHA_TOWER);
...
%	============================= This functions can be found on Eq 3.7 of FH Thesis
     ...
     y=0; %added to eliminate Matlab message
           ...
           for J=1:1:NC
             B21=TT_vector-RZ/C-x/C;
             B22=TT_vector-RZ/C+x/C;
             B21(B21<=0)=0;
             B22(B22<=0)=0; 
             if (B21>=0)
                ICI=ICI+((-1)^(J+1)*RG^(J)*IPULSE(k^(J-1)*B21)+(-1)^(J)*RG^(J)*IPULSE(k^(J)*B22))*exp(-(J+1)*HR*ALPHA_TOWER);
             end
           end
...
%	=============================
    ...
    B3(B3<=0)=0; %Carlos added for solving one problem
	    ...
	    for J=1:1:NC
             ...
             B22(B22<=0)=0;
             if (B21>=0)
              ICI=ICI+((-1)^(J+1)*RG^(J)*k^(J-1)*DIPULSE(k^(J-1)*B21)+(-1)^(J)*RG^(J)*k^(J)*DIPULSE(k^(J)*B22))*exp(-(J+1)*HR*ALPHA_TOWER);
              if not(isreal(ICI))
               disp('Non-real value on the ERADP routine') 
              end
             end
	   ...
	   for J=1:1:NC
           ...
           B22=TT-RZ/C+x/C;
            ...
            if (B21>=0)
             ...
             B21=k^(J-1)*B21;
             B22=k^(J)*B22;
             ICII_1=INTPULSE(A1,B21);
             ICII_2=INTPULSE(A1,B22);
		     ICI=ICI+(ICII_1.*(-1)^(J+1)*RG^(J)*k^(1-J)+ICII_2.*(-1)^(J)*RG^(J)*k^(-J))*exp(-(J+1)*HR.*ALPHA_TOWER);
            end
       end
    y=(ZS-x).*ICI./(RZ.^5).*QEH;
